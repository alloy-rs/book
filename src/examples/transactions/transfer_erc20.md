<!-- DO NOT EDIT THIS FILE. IT IS GENERATED BY RUNNING `./scripts/update.sh` -->
<!-- ANY CHANGES MADE TO THIS FILE WILL BE OVERWRITTEN -->
<!-- LATEST UPDATE: https://github.com/alloy-rs/examples/tree/18184f3caec922bd5ba358f389b5bfea365ef444 -->

## Example: `transfer_erc20`
{{#include ../../templates/transactions/transfer_erc20.md}}
### Usage

To run this example:

- Clone the [examples](https://github.com/alloy-rs/examples) repository: `git clone git@github.com:alloy-rs/examples.git`
- Run: `cargo run --example transfer_erc20`

```rust,ignore
{{#include ../../../lib/examples/examples/transactions/examples/transfer_erc20.rs}}
```

Find the source code on Github [here](https://github.com/alloy-rs/examples/tree/18184f3caec922bd5ba358f389b5bfea365ef444/examples/transactions/examples/transfer_erc20.rs).
